...
public class ComissaoActivity extends AppCompatActivity {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if ( snapshot.exists() ){
                    for ( DataSnapshot dado : snapshot.getChildren() ){
                        ...
                        if ( agendamento.getStatus() != 0 && agendamento.getIdProfessor().equals(idFuncionario)){
                    ...
                    mainBinding.comissaoText.setText(String.format("Comissão do Período: R$ %.2f", ganhoTotal));
    ...
    private void adicionarAgendamento(AgendamentoModel agendamento){
        agendamentos.add(agendamento);
        if ( agendamento.getStatus() == 2){
            float valor;
            if ( agendamento.getPacote().equals("12 Sessões / 260 Reais")){
                valor = 21.66f;
            }else{
                valor = 26.25f;
            }

            float ganhoReal = valor * (Float.parseFloat(String.valueOf(comissao))/100);

            ganhoTotal +=  ganhoReal;
        }
    }
}